...
def test_headers_and_overdue_rule(tmp_path: Path) -> None:
    ...
    assert [ws.cell(1, col).value for col in range(1, 7)] == list(HEADERS)
    assert ws["A1"].value == "任务"
    assert ws["B1"].value == "任务摘要"
    assert ws["C1"].value == "预估完成节点"
    assert ws["D1"].value == "实际完成节点"
    assert ws["E1"].value == "超时原因"
    assert ws["F1"].value == "任务责任人"

    tasks = {ws.cell(row, 1).value: row for row in range(2, ws.max_row + 1) if ws.cell(row, 1).value}
    assert "PAB060·零件到位" in tasks
    row = tasks["PAB060·零件到位"]
    assert ws.cell(row, 3).value.date() == date(2026, 9, 24)
    assert ws.cell(row, 4).value is None
    assert ws.cell(row, 6).value == "王杰"
    assert "协助人：吴晨阳" in ws.cell(row, 2).value

    row_180 = tasks["PAB180·零件到位"]
    assert ws.cell(row_180, 3).value.date() == date(2026, 10, 15)
    assert "国庆假期" in ws.cell(row_180, 2).value

    formulas = []
    for cf_range in ws.conditional_formatting._cf_rules:
        assert "D2" in str(cf_range)
        for rule in ws.conditional_formatting._cf_rules[cf_range]:
            formulas.extend(rule.formula)
    assert any("D2>C2" in formula for formula in formulas)
...
def test_source_covers_both_phases(tmp_path: Path) -> None:
    ...
    names = [
        ...
        if ws.cell(row, 1).value and not str(ws.cell(row, 1).value).startswith("说明")
    ]
